...
// ============== CONFIGURATION ==============
// WiFi Settings
const char* WIFI_SSID = "ACT103708193870";
const char* WIFI_PASSWORD = "96836853";
...
// Pin Definitions - MATCH YOUR HARDWARE WIRING!
const int ECG_PIN = 34;       // AD8232 OUTPUT -> GPIO34 (D34)
const int LO_MINUS_PIN = 2;   // AD8232 LO- -> GPIO2
const int LO_PLUS_PIN = 4;    // AD8232 LO+ -> GPIO4
const int LED_PIN = 13;       // Use GPIO13 for LED (GPIO2 is used for LO-)
...
void handleCommand(String payload, String source) {
  ...
  if (payload.startsWith("{")) {
    ...
    if (strcmp(event, "start-recording") == 0 || 
        ...
        strcmp(cmd, "start") == 0) {
      String sid;
      if (doc["sessionId"].is<const char*>() && strlen(doc["sessionId"]) > 0) {
        sid = doc["sessionId"].as<const char*>();
      } else {
        sid = "session-" + String(millis());
      }
      startRecording(sid);
      return;
    }
...
// ============== ECG SAMPLING (SIMPLE - LIKE ORIGINAL CODE) ==============
void sampleECG() {
  // Read ECG signal (same as original HeartWise_ESP32_READY.ino)
  int rawValue = analogRead(ECG_PIN);
  float voltage = (rawValue / (float)ADC_MAX) * VOLTAGE_REF;
  
  // Convert to millivolts (centered around 1.65V) - EXACTLY like original
  ...
  // Visual feedback (blink every 0.5 sec)
...
float calculateQuality(float voltage) {
  // Simple quality metric based on signal amplitude (same as original)

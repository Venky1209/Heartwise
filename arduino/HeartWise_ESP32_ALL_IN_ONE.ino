...
<head>
  ...
  <title>HeartWise ECG Setup</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .card {
      ...
      max-width: 420px;
      width: 100%;
      box-shadow: 0 20px 60px rgba(0,0,0,0.5), 0 0 40px rgba(240,171,252,0.1);
    ...
    button {
      ...
      border: none;
      border-radius: 12px;
      background: linear-gradient(135deg, #F0ABFC 0%, #E879A8 100%);
      color: #1a1a2e;
      font-size: 16px;
      font-weight: 700;
      cursor: pointer;
    ...
    .current-config span { color: #F0ABFC; }
    .networks { margin-bottom: 14px; }
    .network-item {
      padding: 10px 14px;
      background: rgba(255,255,255,0.04);
      border-radius: 8px;
    ...
    .network-item:hover { background: rgba(240,171,252,0.1); }
    .signal { color: #86EFAC; font-size: 12px; }
  </style>
</head>
<body>
  <div class="card">
    ...
    <div id="currentConfig" class="current-config" style="display:none">
      ...
      WiFi: <span id="curSSID">-</span> | 
      Server: <span id="curIP">-</span>:<span id="curPort">-</span>
    </div>
    ...
    <form id="configForm" action="/save" method="POST">
      ...
      <div class="section">
        <div class="section-title">🖥️ HeartWise Server</div>
        ...
        <div class="hint">Run <code>ipconfig</code> (Windows) or <code>ifconfig</code> (Mac) to find your computer's IP</div>
        <label for="server_port">Server Port</label>
        <input type="number" id="server_port" name="server_port" 
               placeholder="5001" value="5001" min="1" max="65535">
      </div>
  ...
  <script>
    ...
    fetch('/config').then(r=>r.json()).then(c => {
      if (c.ssid) {
        document.getElementById('curSSID').textContent = c.ssid;
        document.getElementById('curIP').textContent = c.server_ip || '-';
        document.getElementById('curPort').textContent = c.server_port || '5001';
        document.getElementById('currentConfig').style.display = 'block';
        document.getElementById('ssid').value = c.ssid;
        document.getElementById('server_ip').value = c.server_ip || '';
        document.getElementById('server_port').value = c.server_port || 5001;
      }
    });
    ...
    fetch('/scan').then(r=>r.json()).then(networks => {
      ...
      if (networks.length === 0) {
        list.innerHTML = '<div style="color:#777;font-size:12px;margin-bottom:10px">Scanning networks...</div>';
        return;
      }
      networks.forEach(n => {
        const div = document.createElement('div');
        div.className = 'network-item';
        div.innerHTML = `<span>${n.ssid}</span><span class="signal">${n.rssi}dBm ${n.secure?'🔒':''}</span>`;
    ...
    document.getElementById('configForm').onsubmit = function(e) {
      ...
      fetch('/save', { method:'POST', body:data, headers:{'Content-Type':'application/x-www-form-urlencoded'} })
        .then(r=>r.json())
        .then(r => {
          const msg = document.getElementById('statusMsg');
          msg.className = 'status success';
          msg.textContent = '✅ Configuration saved! Device will restart in 3 seconds...';
...
void setup() {
  ...
  if (hasStoredConfig()) {
    ...
    if (connectToWiFi()) {
      // WiFi connected — continue to normal operation
      setupWebSocket();
    } else {
...
void startConfigPortal() {
  ...
  // Start scanning WiFi networks in the background
  ...
  // Captive portal detection — redirect common endpoints
  configServer.on("/generate_204", HTTP_GET, handleConfigPage);   // Android
  configServer.on("/fwlink", HTTP_GET, handleConfigPage);          // Microsoft  
  configServer.on("/hotspot-detect.html", HTTP_GET, handleConfigPage); // Apple
  configServer.on("/library/test/success.html", HTTP_GET, handleConfigPage); // Apple
  configServer.on("/connecttest.txt", HTTP_GET, handleConfigPage);  // Microsoft
  configServer.onNotFound(handleConfigPage);  // Everything else
...
void handleSaveConfig() {
  ...
  configServer.send(200, "application/json", 
    "{\"success\":true, \"message\":\"Config saved, restarting...\"}");
